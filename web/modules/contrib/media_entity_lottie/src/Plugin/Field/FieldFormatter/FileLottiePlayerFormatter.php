 ...
 *   description = @Translation("Display the file using an HTML5 lottie-player
 *   tag."), field_types = {
 *     "file"
 *   }
 * )
...
class FileLottiePlayerFormatter extends FileMediaFormatterBase {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $elements['background_transparent'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Background transparent?'),
      '#default_value' => $this->getSetting('background_transparent'),
      '#description' => $this->t('Keeps animation background transparent.'),
    ];
    $elements['hover'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Hover'),
  ...
  public function settingsSummary() {
    ...
    $summary[] = $this->t('Background: %background', ['%background' => $this->getSetting('background')]);
    $summary[] = $this->t('Background transparent: %background_transparent', ['%background_transparent' => $this->getSetting('background_transparent') ? $this->t('yes') : $this->t('no')]);
    $summary[] = $this->t('Hover: %hover', ['%hover' => $this->getSetting('hover') ? $this->t('yes') : $this->t('no')]);
    $summary[] = $this->t('Mode: %mode', ['%mode' => 'PlayMode.' . ucwords($this->getSetting('mode'))]);
    $summary[] = $this->t('Speed: %speed', ['%speed' => ($this->getSetting('speed') === 1) ? $this->t('normal') : $this->getSetting('speed')]);
    $summary[] = $this->t('Count: %count', ['%count' => ($this->getSetting('count') < 1) ? 'undefined' : $this->getSetting('count')]);
  ...
  protected function prepareAttributes(array $additional_attributes = []) {
    ...
    $attributes->setAttribute('background_transparent', $this->getSetting('background_transparent'));

    if (!$this->getSetting('background_transparent')) {
